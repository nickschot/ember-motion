 ...
 */
function v3Combine(a, b, aScale, bScale) {
...
function quaternionToDegreesXYZ(q /*, matrix, row*/) {
  ...
  return [
    roundTo3Places(Math.atan2(2 * qx * qw - 2 * qy * qz, 1 - 2 * qx2 - 2 * qz2) * conv),
    roundTo3Places(Math.atan2(2 * qy * qw - 2 * qx * qz, 1 - 2 * qy2 - 2 * qz2) * conv),
...
export function decomposeTransform(t) {
  const transformMatrix = [t.a, t.b, 0, 0, t.c, t.d, 0, 0, 0, 0, 1, 0, t.tx, t.ty, 0, 1];
  ...
  if (matrix[0][3] !== 0 || matrix[1][3] !== 0 || matrix[2][3] !== 0) {
    ...
    const transposedInversePerspectiveMatrix = transpose(inversePerspectiveMatrix);
    perspective = multiplyVectorByMatrix(rightHandSide, transposedInversePerspectiveMatrix);
  } else {
    // no perspective
  ...
  if (quaternion[0] < 0.001 && quaternion[0] >= 0 && quaternion[1] < 0.001 && quaternion[1] >= 0) {
    ...
    rotationDegrees = [0, 0, roundTo3Places((Math.atan2(row[0][1], row[0][0]) * 180) / Math.PI)];
  } else {
